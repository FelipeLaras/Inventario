<?php
   ...
   /*QUERY DOS MENUS*/
  //ACESSORIOS
   $acessorios = "SELECT * FROM maneger_dropacessorios where deletar = 0 order by nome asc";
   $result_acessorios = mysqli_query($conn, $acessorios) or die(mysqli_error($conn));
  
  //DEPARTAMENTO
   $departamento = "SELECT * FROM maneger_dropdepartamento where deletar = 0 order by nome asc";
   $result_departamento = mysqli_query($conn, $departamento) or die(mysqli_error($conn));

  //EMPRESA/FILIAL
   $empresa = "SELECT * FROM maneger_dropempresa where deletar = 0 order by nome asc";
   $result_empresa = mysqli_query($conn, $empresa) or die(mysqli_error($conn));
  
  //LOCACAO
  $locacao = "SELECT * FROM maneger_droplocacao where deletar = 0 order by nome asc";
  $result_locacao = mysqli_query($conn, $locacao) or die(mysqli_error($conn));

  //EQUIPAMENTOS 
   $equipamentos = "SELECT * FROM maneger_dropequipamentos where deletar = 0 order by nome asc";
   $result_equipamentos = mysqli_query($conn, $equipamentos) or die(mysqli_error($conn));

  //FUNÇÃO
   $funcao = "SELECT * FROM maneger_dropfuncao where deletar = 0 order by nome asc";
   $result_funcao = mysqli_query($conn, $funcao) or die(mysqli_error($conn));

  //OPERADORA 
   $operadora = "SELECT * FROM maneger_dropoperadora where deletar = 0 order by nome asc";
   $result_operadora = mysqli_query($conn, $operadora) or die(mysqli_error($conn));
  
  //STATUS COLABORADOR 
   $status = "SELECT * FROM maneger_dropstatus where deletar = 0 order by nome asc";
   $result_status = mysqli_query($conn, $status) or die(mysqli_error($conn));
  
  //STATUS EQUIPAMENTO
   $statusequipamento = "SELECT * FROM maneger_dropstatusequipamento where deletar = 0 order by nome asc";
   $result_statusequipamento = mysqli_query($conn, $statusequipamento) or die(mysqli_error($conn));

   //SITUAÇÃO EQUIPAMENTO
   $situacao = "SELECT * FROM maneger_dropsituacao where deletar = 0 order by nome asc;";
   $result_situacao = mysqli_query($conn, $situacao) or die(mysqli_error($conn));

   //ESTADO EQUIPAMENTO
   $estado = "SELECT * FROM maneger_dropestado where deletar = 0 order by nome asc;";
   $result_estado = mysqli_query($conn, $estado) or die(mysqli_error($conn));

   //VERSÕES OFFICE
   $office = "SELECT * FROM maneger_dropoffice where deletar = 0 order by nome asc;";
   $result_office = mysqli_query($conn, $office) or die(mysqli_error($conn));

   //VERSÕES WINDOWS
   $windows = "SELECT * FROM maneger_dropsistemaoperacional where deletar = 0 order by nome asc;";
